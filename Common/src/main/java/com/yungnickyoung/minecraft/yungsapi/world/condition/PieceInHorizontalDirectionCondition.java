...
public class PieceInHorizontalDirectionCondition extends StructureCondition {
    ...
    public static final Codec<PieceInHorizontalDirectionCondition> CODEC = RecordCodecBuilder.create((builder) -> builder
            .group(
                    ...
                    CodecHelper.ROTATION_CODEC.fieldOf("rotation").forGetter(conditon -> conditon.rotation))
    ...
    public boolean passes(StructureContext ctx) {
        ...
        if (structureManager == null) YungsApiCommon.LOGGER.error("Missing required field 'structureManager' for piece_in_horizontal_direction condition!");
        ...
        if (structureManager == null || pieces == null || rotation == null || pieceEntry == null) return false;
        ...
        for (PieceEntry otherPieceEntry : pieces) {
            ...
            if (otherPiece.getElement() instanceof SinglePoolElement singlePoolElement) {
                ...
                StructureTemplate otherStructureTemplate = ((SinglePoolElementAccessor)singlePoolElement).callGetTemplate(structureManager);

                // Iterate our target pieces and check for a match with otherPiece
                for (ResourceLocation matchPieceId : matchPieces) {
                    StructureTemplate structureTemplate = structureManager.getOrCreate(matchPieceId);
