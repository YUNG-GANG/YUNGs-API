...
public class LocateReplacerDataPackResources extends AbstractPackResources {
    private static final ResourceLocation TAG_LOCATION = ResourceLocation.fromNamespaceAndPath("c", "tags/worldgen/structure/hidden_from_locator_selection.json");
    ...
    public static final PackMetadataSection METADATA = new PackMetadataSection(Component.translatable("dataPack.yungsapi.locate_replacer.description"), SharedConstants.getCurrentVersion().getPackVersion(PackType.SERVER_DATA), Optional.empty());
    public static final PackLocationInfo LOCATION_INFO = new PackLocationInfo("mod/" + ResourceLocation.fromNamespaceAndPath(YungsApiCommon.MOD_ID, "locate_replacer"),
    ...
    private TagFile makeTagFile() {
        List<TagEntry> entries = LocateReplacerImpl.INSTANCE.getReplacedOrRemovedStructures()
                .map(rk -> TagEntry.optionalElement(rk.location()))
                .toList();
    ...
    private IoSupplier<InputStream> getMetaResource() {
        return () -> {
            String fileContents = PackMetadataSection.CODEC.fieldOf("pack").codec().encodeStart(JsonOps.INSTANCE, METADATA).getOrThrow().toString();
            return new ByteArrayInputStream(fileContents.getBytes(StandardCharsets.UTF_8));
        };
    }
    ...
    @Override
    public @Nullable IoSupplier<InputStream> getResource(PackType packType, ResourceLocation identifier) {
