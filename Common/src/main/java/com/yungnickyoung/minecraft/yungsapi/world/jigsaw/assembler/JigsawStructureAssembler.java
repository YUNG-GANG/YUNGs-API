...
public class JigsawStructureAssembler {
    ...
    private void addChildrenForPiece(PieceEntry pieceEntry) {
        ...
        List<StructureTemplate.StructureBlockInfo> pieceJigsawBlocks = piece.getElement().getShuffledJigsawBlocks(
                this.settings.structureManager,
                piece.getPosition(),
                piece.getRotation(),
                this.settings.rand);
    ...
    private Optional<StructurePoolElement> chooseCandidateFromPool(PieceContext context) {
        ...
        while (candidatePoolElements.size() > 0 && totalWeightSum > 0) {
            ...
            for (Rotation rotation : Rotation.getShuffled(this.settings.rand)) {
                List<StructureTemplate.StructureBlockInfo> candidateJigsawBlocks = chosenPoolElement.getShuffledJigsawBlocks(this.settings.structureManager, BlockPos.ZERO, rotation, this.settings.rand);
                ...
                BoundingBox tempCandidateBoundingBox = chosenPoolElement.getBoundingBox(this.settings.structureManager, BlockPos.ZERO, rotation);
                ...
                if (this.settings.useExpansionHack && tempCandidateBoundingBox.getYSpan() <= 16) {
                    candidateHeightAdjustments = candidateJigsawBlocks.stream().mapToInt((pieceCandidateJigsawBlock) -> {
                        ...
                        int tallestCandidateTargetPoolPieceHeight = candidateTargetPoolOptional.map((structureTemplatePool) -> structureTemplatePool.getMaxSize(this.settings.structureManager)).orElse(0);
                        int tallestCandidateTargetFallbackPieceHeight = candidateTargetFallbackOptional.map((structureTemplatePool) -> structureTemplatePool.getMaxSize(this.settings.structureManager)).orElse(0);
                ...
                for (StructureTemplate.StructureBlockInfo candidateJigsawBlock : candidateJigsawBlocks) {
                    ...
                    BoundingBox rotatedCandidateBoundingBox = chosenPoolElement.getBoundingBox(this.settings.structureManager, candidateJigsawBlockRelativePos, rotation);
                    ...
                    if (isPieceRigid && isCandidateRigid) {
                        ...
                        if (surfaceHeight == -1) {
                            surfaceHeight = this.settings.chunkGenerator.getFirstFreeHeight(context.jigsawBlockPos.getX(), context.jigsawBlockPos.getZ(), Heightmap.Types.WORLD_SURFACE_WG, this.settings.levelHeightAccessor);
                        }
                    ...
                    PoolElementStructurePiece newPiece = this.settings.pieceFactory.create(
                            this.settings.structureManager,
                            chosenPoolElement,
                    ...
                    if (chosenPoolElement instanceof YungJigsawSinglePoolElement yungSingleElement) {
                        StructureContext ctx = new StructureContext.Builder()
                                .structureManager(this.settings.structureManager)
                                .pieces(this.pieces)
    ...
    public static class Settings {
        ...
        /**
         * The level's StructureManager to be used.
         **/
        ...
        /**
         * The level's Random to be used, usually a WorldGenRandom.
         **/
        private Random rand;
        ...
        public Settings structureManager(StructureManager structureManager) {
            this.structureManager = structureManager;
            return this;
        }

        public Settings rand(Random rand) {
            this.rand = rand;
            return this;
        }
        ...
        public Settings pieceFactory(JigsawPlacement.PieceFactory pieceFactory) {
